...
→ Dropout
→ Linear
→ Прогноз Global_active_power на следующий час
```
...
1. Почему задача является задачей анализа временных рядов.
2. Какие признаки были использованы.
3. Как из временного ряда формируются обучающие окна.
4. Почему выбрана LSTM-сеть.
5. Какие метрики используются для регрессии.
6. Как модель прогнозирует потребление на тестовой выборке.
7. Какие ограничения есть у решения и как его можно улучшить.
